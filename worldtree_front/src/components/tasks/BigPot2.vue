<template>
  <div class="page-background">
    <p class="level-dest"> 请用6个数字蒸出24点 </p>
    ...
    <div class="bigpot-container">
      <div class="operators">
        ...
        <button v-for="op in [0, 1, 2]" :key="op" class="operator" @click="setOperator(op)">
          <img :src="require(`@/assets/bigpot/cook${op+3}.png`)" :alt="`operator ${op}`" />
        </button>
      </div>
      <div class="pot-contents">
        <div class="pot-contents-title">蒸笼内部</div>
    ...
    <div v-if="pass" class="modal">
      <div class="modal-content">
        <h2>{{ passMessage }}</h2>
        <p>{{ level !== 4 ? "继续努力，完成第" + (level+1) + "笼！" : "获得笔记残页和贴纸，请及时查看" }}</p>
...
export default {
  ...
  methods: {
    async initGame() {
      try {
        const response = await universalGet('/api/subtask/xiaolongbao/init');
    ...
    async cook() {
      try {
        const response = await universalPost('/api/subtask/xiaolongbao/cook', {
        ...
        if (response.data.code === 0) {
          ...
          if (response.data.data.pass === 1) {
            this.passMessage = this.level === 4 ? '蒸完4笼小笼包，终于可以下班了！' : '小笼包一端上来，同学们都馋哭了！';
            this.pass = true;
          }
        } else {
